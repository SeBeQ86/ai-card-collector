...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "pain type"
      decision: "decision paralysis — too many cards, no priority signal"
    - topic: "persona scope"
      decision: "single named user (self) — auth for protection, not multi-tenancy"
    - topic: "insight"
      decision: "existing tools don't model the hunt; language-edition niche ignored; seller comms friction invisible"
    - topic: "auth model"
      decision: "email + password"
    - topic: "role model"
      decision: "flat — one role, one user"
    - topic: "MVP timeline"
      decision: "3 weeks after-hours; flow: login → add card → score → generate message → list"
    - topic: "message generator"
      decision: "template-based with field substitution; no AI dependency in MVP"
    - topic: "status field"
      decision: "fixed enum: Searching / Contacted / Offer received / Acquired / Abandoned"
  frs_drafted: 11
...
- No AI-generated seller messages — the message generator uses pre-written templates with field substitution only; no language model, no API cost or latency dependency.
...
- **Then** a pre-filled message template appears with the card's name, language edition, and price limit substituted in, ready to copy

#### Acceptance Criteria
- Message is generated from a template — no network call required
- Both English and Portuguese templates are available
- Message text is copyable with one action

## Quality cross-check

Ran at close of shape session. One gap surfaced and accepted:

- **Timeline-cost acknowledged — WARNING**: The 3-week MVP estimate (confirmed in Step 3) conflicts with the hard deadline of 2026-07-05 (11 days from today). No scope cuts were declared to bridge the gap. Consequence: the implementation plan may need to defer some FRs to meet the deadline. Recorded as an open question.

## Open Questions

1. **Timeline vs. deadline gap** — The estimated MVP scope is 3 weeks; the hard deadline is July 5, 2026 (11 days). If implementation reveals the full 11-FR list cannot ship by then, which FRs are deferral candidates? Owner: collector. Block: no (shapes implementation plan, not PRD).
